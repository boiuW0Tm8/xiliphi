...
async function getOrCreateFbp() {
    ...
    if (fbp) return fbp; // already present (real or previously set)

    // Give Meta's pixel a short window to set the genuine _fbp.
    for (let i = 0; i < 10; i++) {
        await new Promise(r => setTimeout(r, 100));
        fbp = getCookie('_fbp');
        if (fbp) return fbp; // got Meta's real value
    }

    // Last resort only: fabricate in Meta's required format.
